...
else if($ac == "login")
{
	...
	if($valid == 1)
	{
		$sql = "SELECT d1.id, d1.password, d1.name, d.company_id, d1.date_format, d1.thousands_sep, d1.time_format, d1.decimal_point, d1.avatar, d1.lang_id, d2.currency_id, d.group_id FROM res_user_company d LEFT OUTER JOIN res_user d1 ON(d.user_id = d1.id) LEFT OUTER JOIN res_company d2 ON(d1.company_id = d2.id) LEFT OUTER JOIN res_user_company d3 ON(d1.id = d3.user_id AND d3.status =0) WHERE d.status =0 AND (d1.user_name='".$user."' OR d1.email='".$user."') AND d1.status =0";
		...
		if($dt->getRowCount()>0)
		{
			...
			if($password == $row->getString("password"))
			{
				...
				$appSession->getConfig()->setProperty("root_company_id", $row->getString("root_company_id"));
				$appSession->getConfig()->setProperty("user_group_id", $row->getString("group_id"));
				
				
				$company_id = $row->getString("company_id");
				$root_company_id = $row->getString("root_company_id");
				$appSession->getConfig()->setProperty("company_id", $company_id);
				
				$msg = $appSession->getTier()->createMessage();
				$msg->add("root_company_id", $root_company_id);
