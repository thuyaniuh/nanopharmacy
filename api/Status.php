...
class Status
{
	...
	public function doStatus($id, $table_id, $status_id, $company_id)
	{
		...
		if(count($arr)>0)
		{
			...
			$sql = "UPDATE res_status_line SET status=2 WHERE rel_id='".$id."' AND status =0";
			$this->msg->add("query", $sql);
			...
			$data = "action=28d9aaeb-9535-4939-9eb6-c88f103cd256&id=".$company_id."&message=".$this->appSession->getTool()->urlEncode("type=database.hq.changed&name=res_status_line");
			...
			for($i =0; $i<count($notifications); $i++)
			{
				...
				if($dt != NULL){
					...
					$this->msg->add("query", $sql);
					$this->appSession->getTier()->exec($this->msg);
					$data = "action=28d9aaeb-9535-4939-9eb6-c88f103cd256&id=".$company_id."&message=".$this->appSession->getTool()->urlEncode("type=database.hq.changed&name=res_notification");
					//$ws->sendMessage($data);
