...
if($ac == "delivery_list")
{
	...
	$sql = "SELECT d1.sale_id AS id, d1.name, d1.tel, d1.email, d1.address, d1.description, d1.sale_id, d2.order_no, d2.order_date, (SELECT COUNT(id) FROM sale_product_local WHERE status =0 AND sale_id=d1.sale_id) AS items, (SELECT SUM(quantity * unit_price) FROM sale_product_local WHERE status =0 AND sale_id=d1.sale_id) AS amount, (SELECT name FROM res_status_line WHERE rel_id=d1.id AND status =0 ORDER BY create_date DESC LIMIT 1) AS status_name, d3.name AS ward_name, d4.name AS dist_name, d5.name AS city_name FROM sale_shipping d1 LEFT OUTER JOIN sale_local d2 ON(d1.sale_id = d2.id) LEFT OUTER JOIN res_address d3 ON(d1.address_id = d3.id) LEFT OUTER JOIN res_address d4 ON(d3.parent_id = d4.id) LEFT OUTER JOIN res_address d5 ON(d4.parent_id = d5.id) WHERE d1.user_id='".$user_id."' AND (d1.proccess = 0 OR d1.proccess = 1 OR d1.proccess IS NULL)";
	$sql = $sql." AND d1.start_date>='".$fdate."'";
	$sql = $sql." AND d1.start_date<='".$tdate."'";
	$sql = $sql." ORDER BY d2.start_date ASC";
	$msg->add("query", $sql);
...
}else if($ac == "sale_shipping")
{
	...
	$sql = $sql.", d3.name AS ward_name, d4.name AS dist_name, d5.name AS city_name";
	$sql = $sql." FROM sale_shipping d1";
	$sql = $sql." LEFT OUTER JOIN sale_local d2 ON(d1.sale_id = d2.id)";
	$sql = $sql." LEFT OUTER JOIN res_address d3 ON(d1.address_id = d3.id)";
	$sql = $sql." LEFT OUTER JOIN res_address d4 ON(d3.parent_id = d4.id)";
	$sql = $sql." LEFT OUTER JOIN res_address d5 ON(d4.parent_id = d5.id)";
	...
	if($status_id != "")
	{
		...
		$status->doStatus($id, "sale_local", $status_id, $company_id);
	}
	echo "OK";
