...
}else if($ac == "product_attribute_photo")
{
	...
	$product = new Product($appSession);
	$dt = $product->countProduct($dt);
	$hasChanged = false;
	for($i =0;$i<$dt->getRowCount(); $i++)
	{
		if($dt->getFloat($i, "unit_in_stock")<0)
		{
			$sql = "UPDATE sale_product_local SET status =1 WHERE id ='".$dt->getString($i, "id")."'";
			$msg->add("query", $sql);
			$r = $appSession->getTier()->exec($msg);
			$hasChanged = true;
		}
...
}else if($ac == "order_list_cancel")
{
	...
	$sql = $sql." ORDER BY d1.order_date DESC LIMIT 200";

	$msg->add("query", $sql);
	$dt = $appSession->getTier()->getTable($msg);
	echo $appSession->getTool()->respTable($dt);
		
}else if($ac == "order_list_sale")
{
	$msg = $appSession->getTier()->createMessage();
	...
	$sql = "SELECT d1.id, d1.order_no, d1.order_date, SUM(d.quantity) AS items, SUM(d.quantity * d.unit_price) AS amount FROM sale_product d LEFT OUTER JOIN sale d1 ON (d.sale_id = d1.id) WHERE d.status =0 AND d1.status =0 AND d1.order_date>='".$fdate." 00:00:00' AND d1.order_date<='".$tdate." 23:59:59'";
	if($customer_id != "")
	{
		$sql = $sql." AND d1.customer_id='".$customer_id."'";
	}else{
	...
	$sql = $sql." GROUP BY d1.id, d1.order_no, d1.order_date ORDER BY d1.order_date DESC";
	$sql = $sql." LIMIT 200";
	$msg->add("query", $sql);
...
}else if($ac == "order_list_company")
{
	...
	{
		$msg = $appSession->getTier()->createMessage();
		$sql = "SELECT m.id, d1.code, d1.name, lg.description AS name_lg, d2.document_id, d3.name AS unit_name, d4.code AS currency_code, m.quantity, m.unit_price, m.unit_id, m.currency_id, d1.description, m.attribute_id, d5.name AS attribute_name, d6.name AS attribute_category_name, m.rel_id, m.type_id, d9.name AS type_name";
		$sql =$sql.", (SELECT SUM(product_count.quantity) FROM product_count LEFT OUTER JOIN res_company ON(product_count.company_id = res_company.id) WHERE product_count.status =0 AND product_count.product_id= d1.id AND (product_count.company_id='".$appSession->getConfig()->getProperty("company_id")."' OR res_company.parent_id='".$appSession->getConfig()->getProperty("company_id")."') AND m.attribute_id = product_count.attribute_id AND product_count.unit_id = m.unit_id) AS unit_in_stock, m.factor, m.second_unit_id, d7.name AS second_unit_name, d12.document_id AS price_document_id, d1.publish AS product_publish, d8.publish AS price_publish, d8.unit_price AS product_price";
...
}else if($ac == "product_list_group_id")
{
	...
	$sql = $sql." AND m.group_id='".$group_id."' AND m.status =0";
	$sql1 = $sql." AND d5.type='PRODUCT'";
	if($search != "")
	{
		$search = $appSession->getTool()->replace($search, "'", "''");
