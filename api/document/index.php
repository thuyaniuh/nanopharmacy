...
}else if($ac == "download" || $ac == "img")
{
	...
	if(file_exists($local_file) && is_file($local_file))
	{
		...
		if($width != "" || $height != "")
		{
			...
			{
				$img = $appSession->getTool()->image_resize($local_file, intval($width), intval($height), $file_resize);
				$local_file = $file_resize;
			}
		}
		...
		$local_file = ABSPATH."assets/img/ico/gif.jpg";
		if(file_exists($local_file) && is_file($local_file))
		{
			$total     = filesize($local_file);
			$blocksize = (2 << 20); //2M chunks
