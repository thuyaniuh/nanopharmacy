...
class Sale
{
	...
	public function checkSaleService($sale_id, $customer_id)
	{
		...
		if(count($arr)<=0)
		{
			$sql = "SELECT d1.service_id, d2.percent, d2.value, d2.category_id, d3.operator, d3.sequence, d5.id FROM account_service_setting d1 LEFT OUTER JOIN account_service d2 ON(d1.service_id = d2.id) LEFT OUTER JOIN account_service_category d3 ON(d2.category_id = d3.id) LEFT OUTER JOIN account_service_setting_rel d4 ON(d1.id = d4.setting_id) LEFT OUTER JOIN sale_product_local d5 ON(d4.rel_id = d5.product_id) WHERE d1.status =0 AND d2.status =0 AND d4.status =0 AND d1.type='SALE' AND d5.status =0 AND d5.sale_id='".$sale_id."'";
			$this->msg->add("query", $sql);
			$arr = $this->appSession->getTier()->getArray($this->msg);
			for($i=0; $i<count($arr); $i++)
			{
				$rel_id = $arr[$i][6];
				$service_id = $arr[$i][0];
				$sql = "SELECT d1.id FROM account_service_line_local d1 WHERE d1.rel_id = '".$rel_id."' AND d1.service_id='".$service_id."'";
	...
	public function productListQuantity($sale_id)
	{
		
		$sql = "SELECT m.rel_id, m.quantity";
		$sql = $sql." FROM sale_product_local m";
		$sql = $sql." WHERE m.status =0 AND m.quantity>0 AND m.sale_id='".$sale_id."'";
	...
	public function totalSalePrice($sale_id)
	{
		...
		$sql = "SELECT  d1.product_id, (d1.quantity * d1.unit_price) AS amount, d1.id";
		...
		$sql = "SELECT d1.percent AS percent, d1.value AS value, d1.category_id, d1.operator, d1.sequence, d1.rel_id";
		$sql = $sql." FROM account_service_line_local d1 LEFT OUTER JOIN sale_product_local d2 ON(d1.rel_id = d2.id AND d2.status =0)";
		$sql = $sql." WHERE d2.sale_id='".$sale_id."' AND d1.status =0 ORDER BY d1.sequence ASC";
		$msg->add("query", $sql);
		...
		{
			$a =  $this->appSession->getTool()->toDouble($products[$i][1]);
			$amount = $amount + $a;
			
			for($j =0; $j<count($serviceList); $j++)
			{
				if($serviceList[$j][5] != $products[$i][2])
				{
					continue;
				}
				$a = ($a * floatval($serviceList[$j][0])) + floatval($serviceList[$j][1]);
				if($serviceList[$j][3] == "+")
				{
					$amount =  $amount + $a;
				}else if($serviceList[$j][3] == "-")
				{
					$amount =  $amount -  $a;
				}else if($serviceList[$j][3] == "*")
				{
