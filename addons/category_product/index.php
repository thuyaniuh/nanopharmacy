...
$arr = $appSession->getTool()->split($category_ids, ";");
...
for ($i = 0; $i < count($arr); $i++) {
    ...
    $items = findChild($dt_category, $arr[$i]);
    if ($items != "") {
        $items = $items . "," . $$arr[$i];
    } else {
        $items =  $arr[$i];
...
$arr = $appSession->getTool()->split($range_price, ";");
if (count($arr) > 1) {
    $sql = $sql . " AND d5.unit_price>=" . $arr[0] . " AND d5.unit_price<=" . $arr[1];
}
