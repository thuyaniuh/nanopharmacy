...
<script>
  function activeAccount() {
    ...
    var _url = "<?php echo URL; ?>api/account/?ac=verify_user_code&id=" + encodeURIComponent('<?php echo $id;?>');
    _url = _url + "&code=" + encodeURIComponent(code);
    loadPage('contentView', _url, function(status, message) {
      if (status == 0) {
        if (message.indexOf("OK") != -1) {
          
          document.location.href = '<?php echo URL; ?>reset/?id=' + encodeURIComponent('<?php echo $id;?>');
