...
$page_id = "customer";
$table_name = "customer";
$columns = ["code", "name", "phone", "email", "address", "category_id", "vat", "commercial_name"];
...
$sql= $sql." WHERE d1.status =0 AND d1.company_id='".$appSession->getConfig()->getProperty("company_id")."'";
$sql = $sql. " AND d2.id IS NULL";

$msg->add("query", $sql);
$pending = $appSession->getTier()->getValue($msg);
...
for($i =0; $i<count($employeeList); $i++)
{
	...
	$ids = $ids." d2.employee_id='".$employeeList[$i][0]."'";
}
...
$msg->add("query", $sql);
$dt_customer = $appSession->getTier()->getTable($msg);

$dt = $appSession->getTool()->selectDistinctTable($dt_customer, ["id", "code", "name", "phone", "email", "vat", "address"]);
...
$sql = "SELECT d1.rel_id, COUNT(d1.id) AS c FROM crm_activity d1 WHERE d1.status =0";
if($ids != "")
{
	$sql = $sql." AND (".$ids.")";
}else{
...
$msg->add("query", $sql);
$dt_activities = $appSession->getTier()->getTable($msg);
...
for($i =0; $i<$dt->getRowCount(); $i++)
{
	...
	$ids = $ids." d1.customer_id='".$dt->getString($i,"id")."'";
}

$sql = "SELECT d1.customer_id, COUNT(d1.id) AS c FROM sale d1 WHERE d1.status =0";
if($ids != "")
{
	$sql = $sql." AND (".$ids.")";
}else{
...
$sql = $sql." GROUP BY d1.customer_id";
$msg->add("query", $sql);
	...
	<div class="row align-items-center mx-0">
		<div class="col-12 col-md-6">
			<div class="title_inner d-flex">
				...
				{
					?>
					<a href="javascript:loadModule('customer_pending')">Pending: <?php echo $pending;?></a>
					<?php
				}
				?>
			</div>
		...
		<div class="col-12  col-md-6 ">
			...
			<div class="form-group d-flex">
					...
					<select class="form-control">
						...
						{
						?>
						<option value="<?php echo $employeeList[$i][0];?>"><?php echo $employeeList[$i][1];?>. <?php echo $employeeList[$i][2];?></option>
						<?php
						}
						?>
					</select>
		...
		<!-- Food Items Tab customers Start -->
			...
			<div class="order_list">
				  ...
				  <table class="table table-bordered table-hover table-sm">
					...
					<td class="text-left " ><?php echo $appSession->getLang()->find("Saleman");?></td>
					<td class="text-left " ><?php echo $appSession->getLang()->find("Orders");?></td>
					<td class="text-left " ><?php echo $appSession->getLang()->find("Activities");?></td>
				...
				<tbody>
					...
					for($i =0; $i<$dt->getRowCount(); $i++)
					{
						...
						$saleman = "";
						for($j =0; $j<$dt_customer->getRowCount(); $j++)
						{
							if($dt_customer->getString($j, "id") == $id){
								if($saleman != "")
								{
									$saleman = $saleman.";";
								}
								$saleman = $saleman.$dt_customer->getString($j, "employee_code").". ".$dt_customer->getString($j, "employee_name");
							}
						}
						$activities = "";
						for($j =0; $j<$dt_activities->getRowCount(); $j++)
						{
							if($dt_activities->getString($j, "rel_id") == $id)
							{
								$activities = $dt_activities->getString($j, "c");
								break;
							}
						}
						$orders = "";
						for($j =0; $j<$dt_sale->getRowCount(); $j++)
						{
							if($dt_sale->getString($j, "customer_id") == $id)
							{
								$orders = $dt_sale->getString($j, "c");
								break;
							}
						}
					...
					<tr class="animate__animated animate__fadeInUp wow">
						...
						<td nowrap="nowrap" class="text-left " ><?php echo $saleman;?></td>
						<td nowrap="nowrap" class="text-left " ><a href="javascript:loadView('addons/transaction/?customer_id=<?php echo $id;?>')"><?php echo $orders;?></a></td>
						<td nowrap="nowrap" class="text-left "><?php echo $activities;?> <a href="javascript:loadView('addons/activities/?rel_id=<?php echo $id;?>')"><img src="<?php echo URL;?>assets/images/add.png"/></a></td>
...
function addNew()
{
	var _url = '<?php echo URL;?>addons/customer/?ac=form';
...
<div class="page_title">
	<div class="row align-items-center mx-0">
		...
		<div class="col-12 col-sm-12 col-md-12 col-lg-2 col-xl-2 p-0 d-flex">
			<button type="button" class="btn btn-outline-primary" onclick="loadModule('customer')"><?php echo $appSession->getLang()->find("Back");?></button>
		</div>
	</div>
</div>
